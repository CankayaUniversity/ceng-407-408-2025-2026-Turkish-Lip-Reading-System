...
def predict_video(model, video_path, device):
    ...
    video_tensor = lip_reading_dataset._load_video(video_path)
    
    if video_tensor.size(0) == 0:
        return "Error: Could not read video"
    
    input_batch = video_tensor.unsqueeze(0).unsqueeze(0).to(device)
...
predicted_text = predict_video(model, test_video, device)
actual_text = lip_reading_dataset._load_alignments(alignment_files[0])
